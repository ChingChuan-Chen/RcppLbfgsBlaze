...
/**
 *  Define BlazeVector and BlazeMatrix for easy use.
 */
using BlazeVector = blaze::CustomVector<double, blaze::aligned, blaze::padded>;
using BlazeMatrix = blaze::CustomMatrix<double, blaze::aligned, blaze::padded, blaze::columnMajor>;
...
struct lbfgs_parameter_t {
     ...
     *  Set it to 0.0 and use past-delta-based test for non-smooth functions.
    ...
    /**
     * A parameter to ensure the global convergence for non-convex functions.
 ...
 * variables.
 */
typedef double (*lbfgs_evaluate_t)(void *instance, const BlazeVector &x, BlazeVector &g);
 ...
 *  upper bound of the step size to search in, provided with the beginning
 * values of variables before the line search, and the current step vector (can
 * be descent direction). A client program can implement this function for more
 ...
 *  @retval double      The upper bound of the step in current line search
 * routine, such that (stepbound * d) is the maximum reasonable step.
 */
typedef double (*lbfgs_stepbound_t)(void *instance, const BlazeVector &xp, const BlazeVector &d);
 ...
 */
typedef int (*lbfgs_progress_t)(void *instance, const BlazeVector &x, const BlazeVector &g, const double fx,
...
/**
 * Line search method for smooth or non-smooth functions.
 ...
 *
 *  @see
 *      Adrian S. Lewis and Michael L. Overton. Non-smooth optimization
 *      via quasi-Newton methods. Mathematical Programming, Vol 141,
 *      No 1, pp. 135-163, 2013.
 */
inline int line_search_lewisoverton(BlazeVector &x, double &f, BlazeVector &g, double &stp, const BlazeVector &s,
                                    const BlazeVector &xp, const BlazeVector &gp, const double stpmin,
                                    const double stpmax, const callback_data_t &cd, const lbfgs_parameter_t &param) {
    int count = 0UL;
 ...
 *              4. g(x) is either the gradient or sub gradient;
 ...
 * compatible with
 * ::lbfgs_stepbound_t to provide an external upper bound for step size, and
 ...
 *                          upper bound of the step size to search in, provided
 ...
 */
inline int lbfgs_optimize(BlazeVector &x, double &f, lbfgs_evaluate_t proc_evaluate, lbfgs_stepbound_t proc_stepbound,
                          lbfgs_progress_t proc_progress, void *instance, const lbfgs_parameter_t &param) {
    int ret, k, ls;
    std::size_t i, j, end, bound;
    ...
    std::unique_ptr<double[], blaze::Deallocate> xp_data(blaze::allocate<double>(padded_size));
    BlazeVector xp(xp_data.get(), n, padded_size);
    std::unique_ptr<double[], blaze::Deallocate> g_data(blaze::allocate<double>(padded_size));
    BlazeVector g(g_data.get(), n, padded_size);
    std::unique_ptr<double[], blaze::Deallocate> gp_data(blaze::allocate<double>(padded_size));
    BlazeVector gp(gp_data.get(), n, padded_size);
    ...
    BlazeVector lm_alpha(lm_alpha_data.get(), m, memory_padded_size);
    std::unique_ptr<double[], blaze::Deallocate> lm_s_padded(blaze::allocate<double>(padded_size * m));
    BlazeMatrix lm_s(lm_s_padded.get(), n, m, padded_size);
    std::unique_ptr<double[], blaze::Deallocate> lm_y_padded(blaze::allocate<double>(padded_size * m));
    BlazeMatrix lm_y(lm_y_padded.get(), n, m, padded_size);
    std::unique_ptr<double[], blaze::Deallocate> lm_ys_data(blaze::allocate<double>(memory_padded_size));
    BlazeVector lm_ys(lm_ys_data.get(), m, memory_padded_size);
    ...
    */
    gnorm_inf = blaze::linfNorm(g);
    xnorm_inf = blaze::linfNorm(x);
        ...
        */
        ...
        while (true) {
            ...
            */
            ...
            y_{k+1} = g_{k+1} - g_{k}.
            */
            blaze::column(lm_s, end) = x - xp;
            blaze::column(lm_y, end) = g - gp;
            ...
            factor).
            */
            ys = blaze::dot(blaze::column(lm_y, end), blaze::column(lm_s, end));
            yy = blaze::sqrNorm(blaze::column(lm_y, end));
            lm_ys[end] = ys;
            ...
            for \alpha is 1.
            This is not for enforcing the PD of the approximated Hessian
            ...
            the BFGS method for non-convex unconstrained optimization problems.
            SIAM Journal on Optimization, Vol 11, No 4, pp. 1054-1064, 2011.
            */
            cau = blaze::sqrNorm(blaze::column(lm_s, end)) * blaze::norm(gp) * param.cautious_factor;
            ...
            if (ys > cau) {
                ...
                */
                ++bound;
                bound = (std::size_t) m < bound ? (std::size_t) m : bound;
                end = (end + 1) % m;
                ...
                for (i = 0; i < bound; ++i) {
                    ...
                    lm_alpha[j] = blaze::dot(blaze::column(lm_s, j), d) / lm_ys[j];
                    /* q_{i} = q_{i+1} - \alpha_{i} y_{i}. */
                    d += (-lm_alpha[j]) * blaze::column(lm_y, j);
                }
                ...
                for (i = 0; i < bound; ++i) {
                    ...
                    beta = blaze::dot(blaze::column(lm_y, j), d) / lm_ys[j];
                    /* \gamma_{i+1} = \gamma_{i} + (\alpha_{j} - \beta_{j}) s_{j}. */
                    d += (lm_alpha[j] - beta) * blaze::column(lm_s, j);
                    j = (j + 1) % m; /* if (++j == m) j = 0; */
                }
            }
